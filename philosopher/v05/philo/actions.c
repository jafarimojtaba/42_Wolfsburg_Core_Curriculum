...
/*   Updated: 2022/06/30 00:10:33 by mjafari          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	living(t_philo *ph)
{
	...
	print_action(ph, "has taken a fork", current_time(ph));
	check_death(ph);
	if (ph->nb_philos == 1)
	{
	...
	print_action(ph, "has taken a fork", current_time(ph));
	print_action(ph, "is eating", current_time(ph));
...
void	sleeping(t_philo *ph)
{
	...
	print_action(ph, "is sleeping", current_time(ph));
	...
	while ((current_time(ph)) - t < ph->sleep_time)
		check_death(ph);
	print_action(ph, "is thinking", current_time(ph));
}
